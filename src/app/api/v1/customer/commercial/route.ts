...
import { customerCommercialCommandSchema } from '@/server/commercial/contract';
...
function deniedStatus(reasonCode?: string): number {
  ...
  if (reasonCode?.includes('STATE') || reasonCode?.includes('MISMATCH')) return 409;
...
export async function POST(request: NextRequest) {
  ...
  if (!viewer.roles.includes('customer')) return noStore({ error: 'CUSTOMER_REQUIRED' }, 403);
  ...
  const input = customerCommercialCommandSchema.safeParse(json);
  if (!input.success) return noStore({ error: 'INVALID_COMMERCIAL_ACCEPTANCE' }, 400);
