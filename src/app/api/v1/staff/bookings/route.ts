...
import { staffBookingCommandSchema } from '@/server/booking/contract';
...
export async function POST(request: NextRequest) {
  ...
  if (!bookingOperationsRoles.some((role) => viewer.roles.includes(role))) {
  ...
  const input = staffBookingCommandSchema.safeParse(json);
  if (!input.success) return noStore({ error: 'INVALID_BOOKING_COMMAND' }, 400);

  try {
    const result = await executeBookingCommand(viewer, input.data, idempotencyKey.data);
    if (result.status === 'denied') return noStore(result, deniedStatus(result.reasonCode));
    return noStore(result, 200);
  } catch {
    return noStore({ error: 'BOOKING_COMMAND_FAILED' }, 500);
  }
}
