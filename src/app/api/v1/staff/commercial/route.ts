...
import { executeCommercialCommand } from '@/server/commercial/command';
import { staffCommercialCommandSchema } from '@/server/commercial/contract';
...
function deniedStatus(reasonCode?: string): number {
  ...
  if (reasonCode?.includes('STATE') || reasonCode?.includes('EDITABLE') || reasonCode?.includes('MISMATCH')) return 409;
  if (reasonCode?.includes('INVALID') || reasonCode?.includes('REQUIRED') || reasonCode?.includes('EXPIRED')) return 400;
  return 403;
}
...
export async function POST(request: NextRequest) {
  ...
  if (!staffAreaRoles.some((role) => viewer.roles.includes(role))) return noStore({ error: 'STAFF_REQUIRED' }, 403);
  ...
  const input = staffCommercialCommandSchema.safeParse(json);
  if (!input.success) return noStore({ error: 'INVALID_COMMERCIAL_COMMAND' }, 400);
  if (
    ['quotation.decide', 'quotation.publish'].includes(input.data.action)
    && !viewer.roles.includes('founder')
  ) {
    return noStore({ error: 'FOUNDER_REQUIRED' }, 403);
  }

  try {
    const result = await executeCommercialCommand(viewer, input.data, idempotencyKey.data);
    if (result.status === 'denied') return noStore(result, deniedStatus(result.reasonCode));
    return noStore(result, 200);
  } catch {
    return noStore({ error: 'COMMERCIAL_COMMAND_FAILED' }, 500);
  }
}
