...
import { executeTravelRequestCommand } from '@/server/travel-request/command';
import { staffTravelRequestCommandSchema } from '@/server/travel-request/contract';
...
function deniedStatus(reasonCode?: string): number {
  ...
  if (reasonCode?.includes('STATE') || reasonCode?.includes('CLAIMED')) return 409;
...
export async function POST(request: NextRequest) {
  ...
  const input = staffTravelRequestCommandSchema.safeParse(json);
  if (!input.success) return noStore({ error: 'INVALID_TRAVEL_REQUEST_COMMAND' }, 400);

  try {
    const result = await executeTravelRequestCommand(viewer, input.data, idempotencyKey.data);
    if (result.status === 'denied') return noStore(result, deniedStatus(result.reasonCode));
    return noStore(result, 200);
  } catch {
    return noStore({ error: 'TRAVEL_REQUEST_COMMAND_FAILED' }, 500);
  }
}
