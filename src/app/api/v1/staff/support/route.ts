...
import { staffSupportCommandSchema } from '@/server/support/contract';
...
function deniedStatus(reasonCode?: string): number {
  ...
  if (
    ...
    || reasonCode?.includes('OWNER')
    || reasonCode?.includes('REQUIRED')
    || reasonCode?.includes('ESCALATION')
  ) return 409;
...
export async function POST(request: NextRequest) {
  ...
  if (!operationsRoles.some((role) => viewer.roles.includes(role))) {
    return noStore({ error: 'SUPPORT_OPERATIONS_AUTHORITY_REQUIRED' }, 403);
  }
  ...
  const input = staffSupportCommandSchema.safeParse(json);
