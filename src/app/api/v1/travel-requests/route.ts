...
import { customerTravelRequestCommandSchema } from '@/server/travel-request/contract';
...
function deniedStatus(reasonCode?: string): number {
  ...
  if (reasonCode?.includes('CONFLICT') || reasonCode?.includes('NOT_EDITABLE')) return 409;
...
export async function POST(request: NextRequest) {
  ...
  const input = customerTravelRequestCommandSchema.safeParse(json);
  if (!input.success) return noStore({ error: 'INVALID_TRAVEL_REQUEST' }, 400);
