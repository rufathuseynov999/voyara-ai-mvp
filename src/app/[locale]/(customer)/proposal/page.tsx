...
import { PublishedProposals } from '@/components/published-proposals';
...
export default async function ProposalPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const { proposals, availableLocales } = await loadCustomerPublishedProposals(viewer, locale);
  ...
  return (
    <main className="screen-page proposal-page" id="main-content" tabIndex={-1}>
      ...
      <PublishedProposals locale={locale} messages={dictionary.proposalLive} proposals={proposals} />
    </main>
  );
}
